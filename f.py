...
# Fonction pour déplacer les fichiers .wav
def proposer_deplacer_fichiers():
    reponse = messagebox.askyesno("Déplacer les fichiers", 
                                  "Voulez-vous déplacer les fichiers .wav du dossier 'C:/Users/oo/Downloads' vers un autre emplacement ?")
    if reponse:  # Si l'utilisateur répond "oui"
        ...
        if dossier_destination:
            ...
            fichiers_wav = [f for f in os.listdir(chemin_source) if f.endswith(".wav")]
            ...
            messagebox.showinfo("Fichiers déplacés", 
                                f"Vos fichiers .wav ont été déplacés vers {dossier_destination}")
